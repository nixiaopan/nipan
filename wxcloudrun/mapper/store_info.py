...
def insert_store_info(store_id, store_name, dsr, openid, ignore=False):
    sql_str = '''
    insert {ignore} into store_info set store_id = "{store_id}", store_name="{store_name}",dsr="{dsr}",openid="{openid}"
    '''.format(store_id=store_id, store_name=db_utils.escape_string(store_name), dsr=dsr, openid=openid,
...
def get_store_data(openid):
    sql_str = '''
            select store_id, store_name, dsr from store_info where openid = '{openid}'
...
def get_store_data_by_store_id(openid, store_id):
    sql_str = '''
            select store_id, store_name, dsr from store_info where openid = '{openid}' and store_id ="{store_id}"
