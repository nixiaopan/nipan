...
@json_response
@get_params
def new_store(request, openid, store_id, store_name, dsr):
    """
    新建商铺
    :request method: POST
    ...
    try:
        is_success = insert_store_info(store_id, store_name, dsr, openid)
        if is_success:
...
def update_store_info(request, openid, store_id):
    ...
    更新商铺信息
    :request method: POST
    :param store_id: 店铺id(最长45位)(必传)
    :param store_name: 店铺名（非必填）(最长45位)
    :param dsr: 店铺dsr评分（非必填）
    :return:code：200成功，450失败，550异常
