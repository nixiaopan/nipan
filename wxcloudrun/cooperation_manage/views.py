...
@json_response
@get_params
def new_cooperation(request,openid):
    """
    新建一次合作
    :request method: POST
    以下所有必填，没有填“无”
    :param goods_id: 商品链接(必填)
    ...
    :param store_name: 商铺名称(必填)
    :param store_drs: 商铺drs评分(必填)
    :param commission_rate: 佣金比例(必填)
    :param pos_price: 坑位费(必填)
    :return:
    {'code': ResponsCode.FAILED, 'data': '', "msg": '新建合作单失败'}
    {'code': ResponsCode.SUCCESS, 'data': '',"msg":'新建合作单成功'}
    ...
    try:
        if not identity_check(openid,IdentityType.BUSINESSES):
            raise PreconditionErr("合作人身份类型错误")
        is_success = insert_cooperation_data(openid,json.loads(request.body))
        if is_success:
            rsp = {'code': ResponsCode.SUCCESS, 'data': '',"msg":'新建合作单成功'}
...
@json_response
@get_params
def apply_for_sample(request,openid,cooperation_id,anchor_name,anchor_phone_number,anchor_shipping_address,sample_count):
    """
    主播才能发送申请领取样品
    :request method: POST
    ...
    try:
        ...
        with cur_db_util.transcontext(True):
            ...
            update_cooperation_status_and_set_shipping_info(cooperation_id,
                CooperationStatus.WAITING_FOR_MERCHANT_SEND_SAMPLE, CooperationStatus.WAITING_FOR_ANCHOR_GET_SAMPLE,anchor_shipping_address,
...
def send_sample(request,openid,cooperation_id,sample_courier_number):
    ...
    try:
        ...
        with cur_db_util.transcontext(True):
            ...
            update_cooperation_status_and_set_sample_courier_number(cooperation_id,CooperationStatus.WAITING_FOR_ANCHOR_TEST_SAMPLE,
                CooperationStatus.WAITING_FOR_MERCHANT_SEND_SAMPLE, sample_courier_number,cur_db_util)
...
def test_sample(request,openid,cooperation_id,test_result,test_comment=None):
    ...
    :param test_result: 测试结果说明（失败请填写备注）
    :return:
    ...
    try:
        ...
        with cur_db_util.transcontext(True):
            ...
            update_cooperation_status_and_set_test_result(cooperation_id,CooperationStatus.SAMPLE_TEST_END,
                CooperationStatus.WAITING_FOR_ANCHOR_TEST_SAMPLE, test_result,test_comment,cur_db_util)
...
def get_cooperation_info_by_status_and_test_result(request,openid,status,page_number,count,test_result=None):
    ...
    try:
        ...
        if identity_type == IdentityType.ANCHOR:
            cooperation_data = get_cooperation_data_by_status(status,openid,"anchor_openid",test_result,start,count)
        elif identity_type == IdentityType.BUSINESSES:
            cooperation_data = get_cooperation_data_by_status(status,openid,"merchant_openid",test_result,start,count)
        else:
